...
{
    public class IgdbQueryBuilder : IIgdbQueryBuilder
    {
        public string GenerateGamesQuery(int page, int pageSize)
        {
            return $@"
        ...
        public string GenerateGamesQueryWithOffset(int offset, int pageSize)
        {
            return $@"
                fields name, aggregated_rating, rating, first_release_date, cover.url, platforms.name;
                where rating != null & platforms != null & cover != null;
        ...
        public string GenerateGamesReleasedBetweenQuery(long startUnix, long endUnix, IEnumerable<int>? platformIds = null, int limit = 500)
        {
