...
namespace GameNewsBoard.Api.Controllers
{
    ...
    {
        private readonly IGNewsService _newsService;
        private readonly ILogger<GNewsController> _logger;

        public GNewsController(IGNewsService newsService, ILogger<GNewsController> logger)
        {
        ...
        public async Task<IActionResult> Get(string platform)
        {
            ...
            try
            {
                ...
                return Ok(new ApiResponse<GNewsResponse>("Notícias carregadas com sucesso.", result));
            }
            catch (Exception ex)
            {
