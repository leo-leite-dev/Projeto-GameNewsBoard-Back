    ...
    public async Task<IActionResult> UploadImage(IFormFile image)
    {
        if (image == null || image.Length == 0)
        {
            return BadRequest(ApiResponseHelper.CreateError(
                "Imagem ausente",
                "Nenhuma imagem foi fornecida para upload."));
        }
        ...
        if (image.Length > 5 * 1024 * 1024)
        {
            return BadRequest(ApiResponseHelper.CreateError(
                "Imagem muito grande",
                "A imagem excede o tamanho máximo permitido de 5MB."));
        }
        ...
        if (!allowedTypes.Contains(image.ContentType))
        {
            return BadRequest(ApiResponseHelper.CreateError(
                "Tipo inválido",
                "O tipo de imagem fornecido não é suportado. Use JPG, PNG ou WebP."));
        }
        ...
        try
        {
            ...
            var result = await _uploadedImageService.RegisterImageAsync(
                userId, stream, image.FileName, image.ContentType, ImageBucketCategory.Tier);

            if (!result.IsSuccess || result.Value is null)
            {
                return ApiResponseHelper.CreateError(
                    "Falha no upload",
                    result.Error ?? "Erro ao registrar a imagem.");
            }

            return Ok(ApiResponseHelper.CreateSuccess(result.Value, "Imagem enviada com sucesso"));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[UploadImage] Erro inesperado ao enviar imagem");
